...
{
public:
  Condition(std::unique_ptr<Expr> condition, std::unique_ptr<Stmt> then_stmt, std::unique_ptr<Stmt> else_stmt)
      : condition_(std::move(condition)), then_stmt(std::move(then_stmt)), else_stmt(std::move(else_stmt)) {}
  void InterpretStmt(std::shared_ptr<Scope> scope) override
  {
...
public:
  ...
  int InterpretExpr(std::shared_ptr<Scope> scope) override { return scope->GetVar(name_); }
...
{
public:
  BinOp(const std::string &op, std::unique_ptr<Expr> left, std::unique_ptr<Expr> right)
